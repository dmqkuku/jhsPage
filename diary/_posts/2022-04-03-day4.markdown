...
1. Generic types에 기술된 모든 타입 파라미터(T, K, V등)을 지우고, bound되어 있다면 bound로. unbound되어 있다면, Object로 채워넣는다.

2. type safety를 보장하기 위해 type cast를 필요한 경우에만 넣어준다. <- "complie time check" and "Execution time cast" 
...
3. extended generic type에서 다형성을 보존하기 위해 bridge method를 만들어준다 <- ?

> what is Bridge Method?

참고 : <a href="https://docs.oracle.com/javase/tutorial/java/generics/erasure.html#:~:text=Generics%20were%20introduced%20to%20the,the%20type%20parameters%20are%20unbounded.">Type Erasure [The Java Tutorial]</a>
</div>

<div class="notice" markdown="1">

generic class, interface를 상속한 경우. raw type으로 사용될 수 있도록 서포트해준다. 사용자가 직접 호출할 수 없다.

Generic Class, Interface를 상속하면서 raw type으로 사용할 경우. 컴파일러가 만들어 준다.

Type Erasure와 마찬가지로 디컴파일러를 사용해도 육안으로 확인할 수 없었다.

참고 : <a href="https://docs.oracle.com/javase/tutorial/java/generics/bridgeMethods.html">Effects of Type Erasure and Bridge Methods. [Oracle]</a>

참고 : <a href="http://stas-blogspot.blogspot.com/2010/03/java-bridge-methods-explained.html">Java Bridge Method Explained</a>
</div>
