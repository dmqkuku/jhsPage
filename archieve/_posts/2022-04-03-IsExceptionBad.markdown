...
이론, 방법론, Guide, Style의 경우 judiciously(알아서 잘 판단해서)하게 받아들여야 한다고 생각합니다.
...
> 누구나 동의하는 상식은 다음과 같다. "함수의 내부를 다른 사용자가 가급적 보지 않고도. 그 함수를 잘 호출해서 사용할 수 있어야 좋은 함수이다."
...
어처피 Java에서는 Exception이 존재하며, Exception 발생을 트리거로 Transaction rollback 시키는 flow는 특정 비지니스 영역에서 분명한 어드밴티지이다.
즉 Java를 사용할 것이라면, Exception을 Cost와 side effect를 잘 알고, Judiciously하게 잘 사용하자.
